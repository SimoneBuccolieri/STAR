...
\begin{document}
\section{Abstract}
Il progetto \textbf{STAR} (Sistema di Tracciamento dello Spazio Aereo e Rotte) è una banca dati pensata per gestire e monitorare, in modo integrato, il traffico aereo sia civile che militare. Questo sistema segue tutte le fasi operative di un volo, dalla pianificazione della rotta e prenotazione degli slot negli aeroporti, fino alla storicizzazione delle operazioni effettuate e alla gestione dei dati telemetrici
...
\end{itemize}
Un Hub Internazionale può inoltre possedere diversi gate, che rappresentano i punti fisici di imbarco.
...
\end{itemize}
Il personale di bordo è gestito da un \textit{Equipaggio} (si tracciano il ruolo e le ore di servizio), mentre i controllori di volo monitorano le fasi di un volo, tracciando quale fase si ha monitorato.  
...
La classe passeggeri rappresenta una categoria di servizio a bordo degli aerei di linea, e possiede un numero di posti a sedere differente, 
caratterizzata da:
\begin{itemize}
...
dato che ogni aeroporto deve appartenere esclusivamente a una delle tre categorie previste.

Sebbene per una gerarchia totale e disgiunta la strategia ottimale sarebbe l'accorpamento 
degli attributi del padre nelle entità figlie, tale approccio risulterebbe impraticabile 
nel nostro contesto. L'entità \texttt{Aeroporto} è infatti coinvolta in numerose relazioni 
(Partenza, Arrivo, Presidio, Origine, Destinazione, Disponibilità), e la sua eliminazione 
richiederebbe la triplicazione di tutte queste associazioni, complicando significativamente 
lo schema e le operazioni di interrogazione.

Si adotta quindi la strategia di mantenimento dell'entità padre \texttt{Aeroporto}, 
collegandola alle tre entità figlie \texttt{HubInternazionale}, \texttt{Regionale} e 
\texttt{BaseMilitare} tramite relazioni di identificazione. Ogni aeroporto mantiene 
gli attributi comuni nell'entità padre, mentre gli attributi specifici sono registrati 
nelle rispettive entità figlie. Questa soluzione centralizza le relazioni e semplifica 
le query, richiedendo però l'implementazione di vincoli applicativi per garantire 
che ogni aeroporto appartenga a esattamente una categoria.

\paragraph{Aereo}
La generalizzazione su \textbf{Aereo} è \emph{totale e sovrapposta}, 
poiché un aeromobile può svolgere contemporaneamente più ruoli operativi 
(ad esempio, un aereo cargo impiegato anche come velivolo militare).

Proprio a causa della natura sovrapposta, è indispensabile mantenere l'entità padre 
\texttt{Aereo}, che contiene gli attributi comuni (matricola, data\_immatricolazione, 
ore\_volo\_accumulate). Le specializzazioni sono rappresentate tramite entità separate 
\texttt{AereoDiLinea}, \texttt{AereoCargo}, \texttt{AereoPrivato} e \texttt{AereoMilitare}, 
ciascuna collegata al padre tramite la matricola (chiave primaria e contemporaneamente 
chiave esterna).

Questa soluzione consente di associare un singolo aereo a più categorie simultaneamente, 
inserendo la stessa matricola in più tabelle figlie, evitando duplicazioni degli attributi 
comuni e garantendo coerenza nei dati condivisi. L'alternativa di eliminare il padre 
introdurrebbe gravi ridondanze e complessità nella gestione degli aeromobili multi-ruolo.

\paragraph{Personale}
La generalizzazione su \textbf{Personale} è \emph{parziale e disgiunta}.
Non tutti i membri del personale ricoprono ruoli specializzati, e ciascun dipendente 
può appartenere al massimo a una sola delle categorie previste (pilota, controllore 
o manutentore).

Si mantiene l'entità padre \texttt{Personale}, contenente gli attributi comuni a tutto 
il personale (identificativo, nome, cognome, date di nascita e assunzione), e si creano 
tre entità figlie \texttt{Pilota}, \texttt{ControlloreVolo} e \texttt{Manutentore}, 
collegate al padre tramite l'identificativo.

La parzialità della gerarchia impone il mantenimento del padre: esistono infatti membri 
del personale (ad esempio, personale amministrativo o di supporto) che non appartengono 
ad alcuna specializzazione. La disgiunzione garantisce che nessun dipendente possa 
ricoprire simultaneamente più ruoli specializzati, mantenendo la separazione tra le 
competenze e le responsabilità operative.

\paragraph{Log di Transito}
La generalizzazione su \textbf{LogTransito} è \emph{totale e disgiunta}.
Ogni rilevazione telemetrica deve appartenere obbligatoriamente a una sola delle tre 
categorie: GPS, dati di volo o errori.

Per questa gerarchia si adotta l'approccio dell'\emph{accorpamento nelle entità figlie}, 
eliminando l'entità padre e creando tre entità indipendenti: \texttt{LogGPS}, 
\texttt{LogDatiVolo} e \texttt{LogErrori}. Ciascuna entità contiene sia gli attributi 
comuni (id\_log, id\_volo, timestamp, altitudine, velocità) sia quelli specifici della 
propria tipologia (ad esempio, latitudine e longitudine per i log GPS, o codice\_errore 
e severità per i log di errore).

Questa scelta è motivata dal numero limitato di relazioni coinvolte (principalmente 
la relazione \texttt{GestioneAlert} per i soli log di errore) e dai vantaggi in termini 
di assenza di valori nulli, prestazioni ottimizzate nelle query e semplicità dei vincoli 
di integrità. Ogni tipo di log può essere gestito indipendentemente senza necessità di 
join o discriminanti.
...
\item \textbf{Applica}(\underline{codice, id\_protocollo, data\_inizio})  
\hspace{3mm} codice $\rightarrow$ BaseMilitare(codice)  
...
\hspace{3mm} codice\_hub $\rightarrow$ HubInternazionale(codice)
...
\item \textbf{Aereo}(\underline{matricola}, data\_immatricolazione, ore\_volo\_accumulate, id\_tipo)  
\hspace{3mm} id\_tipo $\rightarrow$ TipoAereo(id\_tipo)
...
\hspace{3mm} codice\_compagnia $\rightarrow$ CompagniaAerea(codice)  
\hspace{3mm} (codice\_hub, numero\_gate) $\rightarrow$ Gate(codice\_hub, numero\_gate)
...
\item \textbf{AbilitazioneVolo}(\underline{id\_personale}, \underline{id\_tipo}, data\_conseguimento, data\_scadenza)  
...
\item \textbf{FasciaOraria}(\underline{id\_slot}, codice, ora\_inizio, ora\_fine, tipologia, stato\_slot)  
\hspace{3mm} codice $\rightarrow$ Aeroporto(codice)
...
\hspace{3mm} matricola $\rightarrow$ Aereo(matricola)  
\hspace{3mm} aeroporto\_partenza $\rightarrow$ Aeroporto(codice)  
\hspace{3mm} aeroporto\_arrivo $\rightarrow$ Aeroporto(codice)  
...
\item \textbf{Presidio}(\underline{id\_personale}, \underline{codice}, \underline{data\_inizio}, data\_fine*, settore)  
\hspace{3mm} id\_personale $\rightarrow$ ControlloreVolo(id\_personale)  
\hspace{3mm} codice $\rightarrow$ Aeroporto(codice)
...
\item \textbf{Trasporto}(\underline{matricola}, \underline{codice\_container}, \underline{data\_carico}, posizione\_stiva)  
...
\item \textbf{Configurazione}(\underline{matricola}, \underline{id\_classe}, numero\_posti)  
...
\item \textbf{LogGPS}(\underline{id\_log}, id\_volo, timestamp, altitudine, velocita, latitudine, longitudine)  
\hspace{3mm} id\_volo $\rightarrow$ Volo(id\_volo)

\item \textbf{LogDatiVolo}(\underline{id\_log}, id\_volo, timestamp, altitudine, velocita, livello\_carburante, pressione, temperatura)  
\hspace{3mm} id\_volo $\rightarrow$ Volo(id\_volo)

\item \textbf{LogErrori}(\underline{id\_log}, id\_volo, timestamp, altitudine, velocita, codice\_errore, messaggio*, severita)  
\hspace{3mm} id\_volo $\rightarrow$ Volo(id\_volo)

\item \textbf{GestioneAlert}(\underline{id\_personale}, \underline{id\_log}, stato, data\_presa\_carico*)  
